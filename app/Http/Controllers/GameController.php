    ...
    public function processTurnAction($game, $pokerService) {
        ...
        if ($someoneZeroAndCalled) {
            ...
            if ($game->status === 'pre-flop') {
                // Pre-flop: Le tour finit quand la BB (non-dealer) a parlé
                $bbIndex = ($game->dealer_index == 0) ? 1 : 0;
                if ($game->current_turn == $bbIndex) $isPhaseOver = true;
            } else {
                // Post-flop: Le tour finit quand le Dealer a parlé
        ...
        if ($isPhaseOver || in_array($game->status, ['showdown', 'countdown'])) {
            ...
            // Si la phase n'est pas finie, on change simplement de joueur
    ...
    private function advanceGameState($game, $pokerService) {
        ...
        switch ($game->status) {
            ...
            case 'turn':
                ...
                // Post-flop: C'est le NON-dealer qui commence (index inverse du dealer)
