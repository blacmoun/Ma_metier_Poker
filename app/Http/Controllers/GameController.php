...
class GameController extends Controller
{
    ...
    private $showdownDuration = 10;

    /**
     * Collecte les mises sur la table et les ajoute au pot.
     */
    ...
    public function play(Request $request, PokerService $pokerService) {
        ...
        if (!$player || !isset($playersArray[$game->current_turn]) || $playersArray[$game->current_turn]->id !== $player->id) {
    ...
    private function processTurnAction($game, $pokerService) {
        // Logique pour permettre aux deux joueurs de jouer :
        // Si le joueur actuel est celui qui doit finir le tour de table (le 2ème à parler)
        $isLastToPlay = false;

        if ($game->status === 'pre-flop') {
            // Au pre-flop, le dealer commence (SB), donc le tour finit sur l'autre (index opposé au dealer)
            $lastIndex = ($game->dealer_index == 0) ? 1 : 0;
            if ($game->current_turn == $lastIndex) $isLastToPlay = true;
        } else {
            if ($game->current_turn == 1) $isLastToPlay = true;
        }

        if ($isLastToPlay) {
            $this->advanceGameState($game, $pokerService);
        } else {
            // Sinon on passe au joueur suivant (0 -> 1 ou 1 -> 0)
    ...
    private function advanceGameState($game, $pokerService) {
        ...
        switch ($game->status) {
            ...
            case 'pre-flop':
                ...
                $game->update([
                    'status' => 'flop',
                    'community_cards' => $pokerService->deal($deck, 3),
                    'deck' => $deck,
                    'timer_at' => $now->addSeconds($this->turnDuration),
                    'current_turn' => 0
                ]);
            ...
            case 'flop':
                ...
                $game->update([
                    'status' => 'turn',
                    'community_cards' => array_merge($game->community_cards, $pokerService->deal($deck, 1)),
                    'deck' => $deck,
                    'timer_at' => $now->addSeconds($this->turnDuration),
                    'current_turn' => 0
                ]);
            ...
            case 'showdown':
                ...
                foreach ($game->players as $p) {
                    $p->update(['hand' => null, 'current_bet' => 0]);
                }
                break;
        }
    }
    ...
    public function logout() {
        if (session()->has('player_token')) {
            $player = Player::where('session_token', session('player_token'))->first();
            if ($player) {
                $this->resetToWaiting(Game::find($player->game_id));
                $player->delete();
            }
