...
<head>
    <meta charset="UTF-8">
    <title>Poker Pro - English Edition</title>
        ...
        <div class="row">
            <div class="col-md-5">
                <ul class="nav nav-tabs mb-2">
                    ...
                    <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#board">Board</button></li>
                </ul>
                <div class="tab-content border bg-dark p-2" style="height:140px; color: white; overflow-y: auto;">
                    ...
                    <div class="tab-pane fade" id="board">No cards on board.</div>
                </div>
            </div>
...
<script>
    ...
    let cardImages = {}, myHand = [], communityCards = [], pot = 0, isAllIn = false;
    let chipParticles = [];
    ...
    function resetGameState() {
        ...
        myHand = []; communityCards = []; pot = 0; chipParticles = []; isAllIn = false;
    ...
    function updateGameStateLocally(data) {
        // Track previous chips before updating for winner visual
        if (currentStatus !== 'showdown') {
            playerData.forEach((p, i) => { if(p.active) previousChips[i] = p.chips; });
        }

        currentStatus = data.status;
        isAllIn = data.is_all_in; // NEW: from controller
        ...
        // Disable buttons if turn is not yours or if game is running automated all-in sequence
        let canPlay = isMyTurn && playPhase && !isAllIn;

        ['act-call', 'act-raise', 'act-fold', 'act-allin', 'bet-range'].forEach(id => {
            let el = document.getElementById(id); if(el) el.disabled = !canPlay;
        });
    ...
    function draw(){
        ...
        // Timer Bar
        if (timer > 0 && currentStatus !== 'waiting' && !isAllIn) {
        ...
        for(let i=0; i<nPlayers; i++){
            ...
            if(playerData[i] && playerData[i].active){
                ...
                if (currentStatus === 'showdown' && playerData[i].chips > previousChips[i]) {
                    push(); fill("#FFD700"); textAlign(CENTER); textSize(22); textStyle(BOLD);
                    text("👑 WINNER 👑", x, y - avatarH/2 - 30 + sin(frameCount * 0.1) * 10);
                ...
                if (i === dealerIndex) {
                    ...
                    stroke(0); fill(255); ellipse(dx, dy, 25, 25);
                ...
                let isHisTurn = (gameStarted && i === currentTurn && !['showdown', 'countdown'].includes(currentStatus) && !isAllIn);
                    ...
                    if (playerData[i].isMe || currentStatus === 'showdown') {
                        if(playerData[i].hand && playerData[i].hand.length >= 2) {
                            ...
                            if (currentStatus === 'showdown' && playerData[i].handName) {
                                push(); fill(0, 220); rect(cardX - 5, cardY + jch + 5, 80, 22, 5);
                                fill("#FFD700"); textAlign(CENTER, CENTER); textSize(11); text(playerData[i].handName, cardX + 35, cardY + jch + 16); pop();
                            }
                        }
                    } else {
        ...
        if (isAllIn && currentStatus !== 'showdown') {
            statusText = "ALL-IN IN PROGRESS...";
            fill("#ff4444");
        } else if (currentStatus === 'waiting') {
            statusText = "WAITING FOR PLAYERS";
        } else if (currentStatus === 'countdown') {
            statusText = "STARTING IN " + timer + "...";
        } else if (currentStatus === 'showdown') {
            statusText = "SHOWDOWN";
        } else {
            statusText = (playerData[currentTurn]?.isMe ? "YOUR TURN!" : "TURN: " + (playerData[currentTurn]?.name || "..."));
        }
        text(statusText, cx, 45);
    }

    async function joinPlayer(index){
        let name = prompt("Your Name:"); if(!name) return;
